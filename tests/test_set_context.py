...
'''
    data = {
        'DC_CODE': code,
        'DC_SOLUTION': code,
        'DC_SCT': "Ex().check_function_def('shout_echo').check_body().set_context(%s).has_equal_value(name = 'shout_words')" % spec
    }
...
def test_check_dict_comp(spec):
    ...
    data = {
        'DC_CODE': code,
        'DC_SOLUTION': code,
        'DC_SCT': "Ex().check_dict_comp().check_key().set_context(%s).has_equal_value()" % spec
    }
    output = helper.run(data)
    assert output['correct']

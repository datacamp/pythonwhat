...
@pytest.mark.parametrize('stu, passes', [
    ...
    ("[key + str(val) for key,val in x.items() if hasattr(key, 'test') if hasattr(key, 'test')]", False),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if hasattr(key, 'test')]", False),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if isinstance(key, str)]", False),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if isinstance(val, str)]", True),
])
def test_check_list_comp_basic(stu, passes):
    ...
    with helper.verify_sct(passes):
        s.check_list_comp().multi(
            ...
            check_body().has_context(exact_names=True).set_context('a', 2).has_equal_value()
        )
...
@pytest.mark.parametrize('stu, passes, patt, lines', [
    ...
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if isinstance(val, str)]", True, "Great", []),
])
...
test_list_comp(index=1,
    ...
    if lines: helper.with_line_info(res, *lines)

@pytest.mark.parametrize('stu, passes, patt, lines', [
    ("", False, "notcalled", []),
    ("[key for key in x.keys()]", False, "iterincorrect", [1, 1, 17, 24]),
    ("[a + str(b) for a,b in x.items()]", False, "incorrectitervars", [1, 1, 17, 19]),
    ("[key + '_' + str(val) for key,val in x.items()]", False, "bodyincorrect", [1, 1, 2, 21]),
    ("[key + str(val) for key,val in x.items()]", False, "insufficientifs", []), # [1, 1, 2, 41] doesn't work...
    ("[key + str(val) for key,val in x.items() if hasattr(key, 'test') if hasattr(key, 'test')]", False, "notcalled1", [1, 1, 45, 64]),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if hasattr(key, 'test')]", False, "notcalled2", [1, 1, 69, 88]),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if isinstance(key, str)]", False, "incorrect2", [1, 1, 80, 82]),
    ("[key + str(val) for key,val in x.items() if isinstance(key, str) if isinstance(val, str)]", True, "Great", []),
])
...
@pytest.mark.parametrize('stu, passes', [
    ("[a for a in x.items()]", False),
    ("[a for a,b in x.items()]", True)
])

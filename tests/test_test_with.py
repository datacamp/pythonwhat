...
@pytest.mark.parametrize('sct, passes, patt, lines', [
    (
...
@pytest.mark.parametrize('sct, stu, passes', [
    ("Ex().check_with(0).has_context()", "with StringIO() as f1, StringIO() as f2: pass", True),
    ("Ex().check_with(0).has_context()", "with StringIO() as f1: pass", False),
    ("Ex().check_with(0).has_context(exact_names=True)", "with StringIO() as f3, StringIO() as f4: pass", False),
    ("Ex().check_with(0).check_context(0).has_context()", "with StringIO() as f1, StringIO() as f2: pass", True),
    ("Ex().check_with(0).check_context(0).has_context(exact_names=True)", "with StringIO() as f3: pass", False)
])

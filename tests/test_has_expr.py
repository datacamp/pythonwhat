...
@pytest.mark.parametrize('copy, passes', [
    (False, True),
    (True, False),
])
def test_copy_functionality(copy, passes):
    ...
    with helper.verify_sct(passes):
        s.has_equal_value(expr_code = 'a[0] = 3', name = 'a', copy = copy).has_equal_value(expr_code = 'a', name = 'a')

@pytest.mark.parametrize('tol, passes', [
    (0.001, True),
    (0.0001, False)
])
def test_test_custom_equality_func(tol, passes):
    ...
    import numpy as np
    with helper.verify_sct(passes):
        s.check_object('a').has_equal_value(func = lambda x, y: np.allclose(x, y, atol = tol))

...
def test_check_function_def_no_args(stu, passes):
    ...
    with helper.verify_sct(passes):
        s.check_function_def('shout').check_body().set_context('test').has_equal_output()
...
def test_check_function_def_name(stu, passes):
    ...
    with helper.verify_sct(passes):
        s.check_function_def('shout').check_body().set_context('test').has_equal_value(name = 'bigword')
...
def test_check_function_def_args(stu, passes):
    ...
    with helper.verify_sct(passes):
        s.check_function_def('f').multi(
            check_args(0).has_equal_part('name', msg='wrong').has_equal_part('is_default', msg='wrong'),
            check_args(1).has_equal_part('name', msg='wrong').has_equal_part('is_default', msg='wrong').has_equal_value()
        )
...
def test_check_function_equal_part_len(sct, stu, passes):
    res = helper.run({ "DC_CODE": stu, "DC_SOLUTION": 'def f(a, b): pass', "DC_SCT": sct })

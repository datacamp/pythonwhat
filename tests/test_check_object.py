...
@pytest.mark.parametrize('stu, sol', [
...
'''
    ...
    if name == 'i': helper.no_line_info(res)
    else: helper.with_line_info(res, ls, le, cs, ce)
...
def test_several_assignments(diff_assign_data):
    res = helper.run({
        **diff_assign_data,
        "DC_SCT": "Ex().check_object('df').has_equal_value()"
    })
    assert res['correct']
...
def test_several_assignments_2(diff_assign_data):
    res = helper.run({
        **diff_assign_data,
        "DC_SCT": "Ex().check_object('df2').has_equal_value()"
    })
    assert not res['correct']

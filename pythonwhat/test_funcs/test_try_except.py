...
MSG_MISSING = "FMT:The system wants to check the {typestr} but hasn't found it."
MSG_PREPEND = "FMT:Check the {typestr}. "
MSG_MISSING_PART  = "FMT:Have you included a {part} in your {parent[typestr]}?"
...
"""
    ...
    child = check_node('try_excepts', index-1, "{ordinal} try-except block", MSG_MISSING, MSG_PREPEND, state)
    quiet_child = quiet(1, state=child)

    expand_msg = None if expand_message else ""
    multi(body, state=check_part("body", "body", expand_msg=expand_msg, state=child))       # subtests
    ...
    for key,value in handlers.items():
        ...
        check_handler = partial(check_part_index, 'handlers', key, incorrect_part, missing_msg=MSG_MISSING_PART, expand_msg=expand_msg)

        check_handler(state=quiet_child)                       # exists
        multi(value, state=check_handler(state=child))         # subtests

    # test orelse and finalbody
    check = partial(check_part, missing_msg=MSG_MISSING_PART, expand_msg=expand_msg)

    # test orelse 
    if child.solution_parts['orelse']:
        check('orelse', "`else` part", state=quiet_child)                         # exists
        multi(orelse, state=check('orelse', "`else` part", state=child))          # subtests

    # test orelse 
    if child.solution_parts['finalbody']:
        check('finalbody', "`finally` part", state=quiet_child)                   # exists
        multi(finalbody, state=check('finalbody', "`finally` part", state=child)) # subtests

...
def test_expression_output(extra_env = None,
	...
	if incorrect_msg is not None:
		...
		feedback_msg = "Unexpected expression output: expected `%s`, got `%s` with values" + ((" "+str(extra_env)) if extra_env else ".")
		feedback_msg = feedback_msg % (utils.shorten_str(str(out_solution)), utils.shorten_str(str(out_student)))

	Reporter.active_reporter.do_test(eq_map[eq_condition](out_solution, out_student, feedback_msg))

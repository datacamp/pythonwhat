...
class Test(object):
	...
	def __init__(self, failure_msg):
		...
		if (issubclass(type(failure_msg), fb.FeedbackMessage)):
			...
			raise TypeError("Not a valid type for failure_msg: %r" % type(failure_msg))
...
class EnvironmentTest(Test):
	...
	def __init__(self, obj, student_env, solution_env, failure_msg):
		...
		self.failure_msg.add_information("solution", self.solution_env[self.obj])
...
class EquivalentEnvironmentTest(EnvironmentTest):
	...
	"""
	def specific_test(self):
		self.result = (abs(self.student_env[self.obj] - self.solution_env[self.obj]) < 0.5e-8)
...
class EqualEnvironmentTest(EnvironmentTest):
	...
	def specific_test(self):
		...
		"""
		self.result = (self.student_env[self.obj] == self.solution_env[self.obj])
...
class EqualTest(Test):
	...
	def specific_test(self):
		...
		"""
		if type(self.obj1) == type(self.obj2) :

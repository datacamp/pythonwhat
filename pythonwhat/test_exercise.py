...
def test_exercise(sct,
		...
		try:
			...
			if (issubclass(type(err_obj), SyntaxError)):
				if (issubclass(type(err_obj), IndentationError)):
					feedback_msg = "Your code can not be excuted due to an error in the indentation: %s." % str(err_obj)
				else:
					feedback_msg = "Your code can not be excuted due to a syntax error: %s." % str(err_obj)
	...
	state = State(student_code, solution_code, pre_exercise_code, student_environment, solution_environment, raw_student_output)

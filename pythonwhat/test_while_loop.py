...
def test_while_loop(index = 1,
	...
	try:
		lineno_student, test_student, body_student, orelse_student = student_whiles[index]
	except:
		rep.do_test(Test("Define more `while` loops."))
		return

	lineno_solution, test_solution, body_solution, orelse_solution = solution_whiles[index]
	...
	def sub_test(closure, subtree_student, subtree_solution, incorrect_part):
		if closure:
			...
			if expand_message and (failed_before is not rep.failed_test):
				rep.feedback_msg = rep.feedback_msg + " in the " + incorrect_part + " of the `while` loop on line " + str(lineno_student) + "."
